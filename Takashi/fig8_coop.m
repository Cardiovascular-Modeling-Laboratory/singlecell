...
filePattern = fullfile(dataDir, 'fig8_*.mat'); % 新しいファイルパターン
resultFiles = dir(filePattern);
total_time = 72 * 3600;  % 72時間分を秒に変換
% ファイル情報の抽出
file_info = struct('filename', {}, 'fold_time', {});
...
for k = 1:length(resultFiles)
    ...
    if ~isempty(tokens)
        ...
        file_info(end+1) = struct('filename', filename, 'fold_time', fold_time);
    else
        disp(['Could not parse fold_time from filename: ', filename]);
    end
end

% ファイルペアの生成
fold_times = [file_info.fold_time];
unique_times = unique(fold_times); % fold_timeのユニークな値
n_files = length(unique_times);
COOP_matrix = NaN(n_files, n_files); % fold_timeごとのヒートマップ

for i = 1:n_files
    for j = i+1:n_files
        % ペアのファイルを取得
        file1 = file_info(find(fold_times == unique_times(i), 1)).filename;
        file2 = file_info(find(fold_times == unique_times(j), 1)).filename;
...
% ヒートマップのプロット
figure;
h = heatmap(unique_times, unique_times, COOP_matrix, ...
    'Colormap', jet, 'ColorLimits', [0, 1]);

xlabel('Shrink duration 1');
ylabel('Shrink duration 2');
title('COOP Heatmap (Fold Time Pairs)');
h.ColorbarVisible = 'on';

% 結果をテキストで保存
T = array2table(COOP_matrix, 'RowNames', string(unique_times), 'VariableNames', string(unique_times));
writetable(T, 'COOP_results_fold_time.csv', 'WriteRowNames', true);

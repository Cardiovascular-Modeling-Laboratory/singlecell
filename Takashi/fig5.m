...
filePattern = fullfile(dataDir, 'fig5*.mat');
...
for k = 1:length(resultFiles)
    ...
    if ~isempty(tokens)
        stretch_factor_str = tokens{1}{1}; % stretch_factor を文字列として取得
        stretch_factor_num = str2double(stretch_factor_str); % stretch_factor を数値として取得
    else
        disp(['Could not parse stretch_factor from filename: ', resultFiles(k).name]);
        continue; % 次のファイルへ
    end
    ...
    % 結果を保存
    stretch_factors = [stretch_factors; stretch_factor_num]; % 数値として保存
...
% ---- stretch_factor でソート ----
[stretch_factors_sorted, sort_idx] = sort(stretch_factors); % stretch_factor でソート
...
stretch_factors_categorical = categorical(string(stretch_factors_sorted));
stretch_factors_categorical = reordercats(stretch_factors_categorical, string(stretch_factors_sorted));
...
subplot(1, 3, 1);
bar(stretch_factors_categorical, mean_angles_sorted);
...
subplot(1, 3, 2);
bar(stretch_factors_categorical, OOP_values_sorted);
xlabel('Stretch Factor');
ylabel('OOP (unitless)'); % 単位を追加

% グラフ3: Traction Force vs Stretch Factor
subplot(1, 3, 3);
bar(stretch_factors_categorical, traction_forces_sorted);
xlabel('Stretch Factor');

...
filePattern = fullfile(dataDir, 'fig5_v1_*.mat'); % 新しいファイルパターン
...
for k = 1:length(resultFiles)
    ...
    % ファイル名からstretch_lengthとfold_timeを抽出
...
% ファイルペアの生成
stretch_lengths = [file_info.stretch_length];
unique_lengths = unique(stretch_lengths); % stretch_lengthのユニークな値
n_files = length(unique_lengths);
COOP_matrix = NaN(n_files, n_files); % stretch_lengthごとのヒートマップ

for i = 1:n_files
    for j = i+1:n_files
        % ペアのファイルを取得
        file1 = file_info(find(stretch_lengths == unique_lengths(i), 1)).filename;
        file2 = file_info(find(stretch_lengths == unique_lengths(j), 1)).filename;
...
for i = 1:n_files
    for j = i+1:n_files
        if isnan(COOP_matrix(i, j)) && isnan(COOP_matrix(j, i))
            error('Error: NaN found in off-diagonal elements at (%d, %d)', i, j);
        elseif isnan(COOP_matrix(i, j)) % i, j の要素が NaN の場合
            COOP_matrix(i, j) = COOP_matrix(j, i);
        elseif isnan(COOP_matrix(j, i)) % j, i の要素が NaN の場合
            COOP_matrix(j, i) = COOP_matrix(i, j);
        end
    end
end

% 再確認：対角成分以外にNaNが残っていないかチェック
off_diag_elements = COOP_matrix(~eye(size(COOP_matrix)));
...
% ヒートマップのプロット
figure;
h = heatmap(unique_lengths, unique_lengths, COOP_matrix, ...
    'Colormap', jet, 'ColorLimits', [0, 1]);

xlabel('Stretch Length 1');
ylabel('Stretch Length 2');
title('COOP Heatmap (Stretch Length Pairs)');
h.ColorbarVisible = 'on';

% 結果をテキストで保存
T = array2table(COOP_matrix, 'RowNames', string(unique_lengths), 'VariableNames', string(unique_lengths));
writetable(T, 'COOP_results_stretch_length.csv', 'WriteRowNames', true);
